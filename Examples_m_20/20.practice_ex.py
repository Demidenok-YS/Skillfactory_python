"""
Существует довольно много разных способов, алгоритмов и систем шифрования.
...
на другую с заранее заданным сдвигом по алфавиту. Например, мы решаем поменять
текст «Привет, мир!» со сдвигом в один символ. Зашифрованный текст будет «Рсйгёу, нйс!».
То есть следующая буква после «п» идёт «р», следующая после «р» идёт «с» и так далее.
...
1) В первых двух строчках задаем алфавит, по которому будем ходить. Так как символы в
верхнем и нижнем регистре отличаются, а мы не хотим, чтобы шифр ломал регистр, то задается два алфавита.
...
9) Однако, если символ попадает в какой-то алфавит, тогда мы вычисляем
положение этого символа в строке, сдвигаем его на введенное пользователем число.
Остаток от деления на длину алфавита нужен для случая, когда пользователь ввёл
число больше, чем длина алфавита, например, 35. Тогда остаток будет 2, то есть число 35,
